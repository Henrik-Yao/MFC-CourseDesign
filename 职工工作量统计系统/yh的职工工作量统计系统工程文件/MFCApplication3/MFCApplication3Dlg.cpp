...
ON_WM_PAINT()
ON_WM_QUERYDRAGICON()
ON_BN_CLICKED(IDC_BUTTON_LGN, &CMFCApplication3Dlg::OnBnClickedButtonLgn)
END_MESSAGE_MAP()
...
void CMFCApplication3Dlg::OnBnClickedButtonLgn()
{
	// TODO: 在此添加控件通知处理程序代码
	CString nam, spwd;
	GetDlgItemText(IDC_EDIT_UNAME, nam);
	GetDlgItemText(IDC_EDIT_PWD, spwd);
	if (!nam.CompareNoCase(_T("yh")) && !spwd.CompareNoCase(_T("202012182")))
	{
		// ShowWindow(SW_HIDE);
		SystemDlg dlg;
		dlg.DoModal();
